...
def create():
    ...
    if request.method == 'POST':
        ...
        registro = anuncios(form['nome_vendedor'], form['nome_produto'], ['valor_produto'], form['descricao_produto'], form['imagem_url'])
        registro.save()
...
def update(registro_id):
    ...
    if request.method == 'POST':
        form = request.form
        new_data = anuncios(form['nome_vendedor'], form['nome_produto'], form[
                            'valor_produto'], form['descricao_produto'], form['imagem_url'])
...
def delete(registro_id):
    ...
    if request.method == 'POST':
        form = request.form
        novo = Usuario(form['nome'], form['endereco'])
        novo.save()
        comprado_s = True
        nome = novo.nome
        endereco = novo.endereco
        id_atribuido = novo.id
     
    return render_template('dados.html', usuario = usuario, registro=registro, id_atribuido = id_atribuido, comprado_s = comprado_s, nome = nome, endereco = endereco)
